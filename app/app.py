...
def FUN_write_note():
    ...
    text_to_write = predict_project(text_to_write)
    write_note_into_db(session['current_user'], text_to_write)

    return(redirect(url_for("FUN_private")))

def predict_project(text_to_write):
    # temp_text = text_to_write + "(TP-1)"
    Pkl_Filename = "Pickle_LR_Model.pkl"  
    with open(Pkl_Filename, 'rb') as file:
        model_imported = pickle.load(file)

    project_name = model_imported.predict([text_to_write])
    temp_text = f" {text_to_write} - * {project_name[0]} * "
    print(f"Model type:  {type(model_imported)}")
    print(f"Model : {temp_text}")
    temp_text = create_ticket(text_to_write, project_name[0])

    return(temp_text)

def create_ticket(text_to_write, project_name):
    jira = JIRA(basic_auth=('admin', 'password'), options={'server': 'http://192.168.2.209:8080/'})
    project_dict = {
        "Political Discussion": "PD",
        "Machine Learning": "ML",
        "Books": "BK",
        "Cooking": "CK",
        "Movies": "MOV",
    }
    project_key = project_dict.get(project_name)

    
    issue = jira.issue('TP-1')
    ...
    if (new_issue.fields.assignee is None):
        assignee = 'Unassigned'
    else:
        assignee = new_issue.fields.assignee.displayName

    return(f" {text_to_write} - * {project_name} - {new_issue}- {assignee} * ")
